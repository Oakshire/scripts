  ...
  if SpellName == 'Read' then
      local con = CreateConversation()
       AddConversationOption(con, "Quickly scan the recent pages.", "Entry")
        ...
        StartDialogConversation(con, 1, NPC, Spawn, "This ledger is one of many records for the movement of goods in the Qeynos Cache.")
end
end
...
function Entry(NPC, Spawn)
      local con = CreateConversation()
        AddConversationOption(con, "Leave the ledger.","Exit")
        StartDialogConversation(con, 1, NPC, Spawn, "A note scribbled in the margins. \'The lieutenant wants the new orders kept upstairs. For his eyes only!\' ")
end
