...
function Talk(NPC, Spawn)
	if HasCompletedQuest(Spawn, WOOD_ELF_QUEST_4) then
		...
		elseif GetRace(Spawn) == DARK_ELF then
			size = size + 1
			voice_overs[size]= {}
		...
		elseif GetRace(Spawn) == WOOLD_ELF then
		   ...
		   choice = math.random(1,2)
		    if choice == 1 then
            PlayFlavor(NPC, "voiceover/english/woodelf_eco_race_good/ft/eco/good/woodelf_eco_race_good_hail_gf_50387146.mp3", "To be among the highest leaves and branches of a tree is to feel free.", "", 1191446540, 711911709)
            elseif choice == 2 then
