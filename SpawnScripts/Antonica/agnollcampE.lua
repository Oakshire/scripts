...
local spawn1 = math.random(0,1)
    if spawn1 == 1 then
local Camp1Spawn1 = SpawnByLocationID(zone, 133781834) --Gnoll1 (shaman)
    local Camp1Spawn1a = SpawnByLocationID(zone, 133781833) -- Gnoll1(battler)
    end   
local spawn2 = math.random(0,1)
    if spawn2 == 1 then
    local Camp1Spawn2 = SpawnByLocationID(zone, 133781835)--Gnoll2 (shaman)
    else
    local Camp1Spawn2a = SpawnByLocationID(zone, 133781828)-- Gnoll2(battler)
    end
local spawn3 = math.random(0,1)
    if spawn3 == 1 then
    local Camp1Spawn3 = SpawnByLocationID(zone, 133781839)--Gnoll3 (shaman)
    else
    local Camp1Spawn3a = SpawnByLocationID(zone, 133781832)-- Gnoll3(battler)
    end   
local spawn4 = math.random(0,1)
    if spawn4 == 1 then
    local Camp1Spawn4 = SpawnByLocationID(zone, 133781838)--Gnoll4 (shaman)
    else
    local Camp1Spawn4a = SpawnByLocationID(zone, 133781831)-- Gnoll4(battler)
    end
local spawn5 = math.random(0,1)
    if spawn5 == 1 then
    local Camp1Spawn5 = SpawnByLocationID(zone, 133781836)--Gnoll5 (shaman)
    else
    local Camp1Spawn5a = SpawnByLocationID(zone, 133781829)-- Gnoll5(battler)
    end
local spawn6 = math.random(0,1)
    if spawn6 == 1 then
    local Camp1Spawn6 = SpawnByLocationID(zone, 133781837)--Gnoll6 (shaman)
    else
    local Camp1Spawn6a = SpawnByLocationID(zone, 133781830)-- Gnoll6(battler)
    end
...
function Camp1_Stage2(NPC, Spawn) --Stage 2 
  local zone = GetZone(NPC)
    local Camp1_Stage2Spawn1=SpawnByLocationID(zone, 133774038)--elder 1
    local Camp1_Stage2Spawn2=SpawnByLocationID(zone, 133774038)--tormentor 1
    local Camp1_Stage2Spawn3=SpawnByLocationID(zone, 133774038)--elder 2
    local Camp1_Stage2Spawn4=SpawnByLocationID(zone, 133774038)--tormentor 2
...
function Camp1_Stage3(NPC, Spawn) --Stage 3 (Possible Boss) 
  ...
  local choice = math.random(0,100)
    ...
    local Camp1_Stage3Spawn1=SpawnByLocationID(zone, 193116)--Pack Lord
...
function Camp1SpawnCheck_Stage3(NPC, Spawn)  --Stage 2 Living Check
  local zone = GetZone(NPC)
  local Camp1_Stage3Spawn1 = GetSpawnByLocationID(zone, 193116) --Pack Lord
 if IsAlive(Camp1_Stage2Spawn1) == true then
