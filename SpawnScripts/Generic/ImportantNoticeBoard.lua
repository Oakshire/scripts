...
function Discord(NPC,Spawn)
 ...
 function Goal(NPC,Spawn)
    ...
    AddConversationOption(con, "[ Content Status ]", "ContentStatus")
    AddConversationOption(con, "[ Exit ]","CloseConvo")
    StartDialogConversation(con, 1, NPC, Spawn, "Goal: The ability to take what is developed here and apply it to a custom server.  Content across the EMU is focused from Classic to Alter of Malice (AoM) in various states. A future server opporator can take the EQ2EMU tools and apply their own rulesets with the foundation here.")
 end   
...
function QuesttBugs(NPC,Spawn)
    ...
    AddConversationOption(con, "[ Main Menu ]", "MainMenu")
    AddConversationOption(con, "[ Combat Bugs ]", "CombatBugs")
    AddConversationOption(con, "[ Tradeskill Bugs ]", "Tradeskill")
    AddConversationOption(con, "[ Exit ]","CloseConvo")
